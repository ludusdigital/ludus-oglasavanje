---
title: Alati i resursi
description: Kompletna lista alata za SaaS oglasavanje - Google alati, Meta alati, SEO, dizajn, video, landing page, email marketing, analitika i resursi za ucenje.
---

## 1. Google alati (besplatni)

| Alat | URL | Namena |
|------|-----|--------|
| **Google Ads** | [ads.google.com](https://ads.google.com) | Kreiranje i upravljanje Google oglasima |
| **Google Analytics 4** | [analytics.google.com](https://analytics.google.com) | Web analitika — prati saobraćaj, ponašanje, konverzije |
| **Google Tag Manager** | [tagmanager.google.com](https://tagmanager.google.com) | Centralizovano upravljanje svim tagovima (pikselima, skriptama) |
| **Google Search Console** | [search.google.com/search-console](https://search.google.com/search-console) | SEO praćenje — pozicije, indeksiranost, tehničke greške |
| **Google Keyword Planner** | [ads.google.com](https://ads.google.com) (Tools > Keyword Planner) | Keyword research — obim pretrage, konkurencija, predlozi |
| **Google PageSpeed Insights** | [pagespeed.web.dev](https://pagespeed.web.dev) | Testiranje brzine sajta i Core Web Vitals |
| **Google Looker Studio** | [lookerstudio.google.com](https://lookerstudio.google.com) | Besplatan dashboarding alat — vizualizacija podataka |
| **Google Ads Transparency Center** | [adstransparency.google.com](https://adstransparency.google.com) | Vidi oglase konkurencije na Google-u |
| **Google Trends** | [trends.google.com](https://trends.google.com) | Trend pretrage kroz vreme — sezonalnost, popularnost |
| **Google Mobile-Friendly Test** | [search.google.com/test/mobile-friendly](https://search.google.com/test/mobile-friendly) | Proveri mobilnu prilagođenost sajta |
| **Rich Results Test** | [search.google.com/test/rich-results](https://search.google.com/test/rich-results) | Testiranje schema markup-a |

---

## 2. Meta alati (besplatni)

| Alat | URL | Namena |
|------|-----|--------|
| **Meta Business Suite** | [business.facebook.com](https://business.facebook.com) | Upravljanje FB stranicom, IG nalogom, objavama |
| **Meta Ads Manager** | [facebook.com/adsmanager](https://facebook.com/adsmanager) | Kreiranje i upravljanje FB/IG oglasima |
| **Meta Events Manager** | [facebook.com/events_manager](https://facebook.com/events_manager) | Praćenje pixel-a, konverzija, event-ova |
| **Meta Pixel Helper** | Chrome extension | Testiranje Meta Pixel-a na sajtu |
| **Facebook Ad Library** | [facebook.com/ads/library](https://facebook.com/ads/library) | Vidi SVE aktivne oglase bilo kog brenda na Facebooku |
| **Meta Creative Hub** | [facebook.com/ads/creativehub](https://facebook.com/ads/creativehub) | Kreiranje i preview mock-up oglasa |

---

## 3. SEO alati

### Plaćeni

| Alat | Cena (okvirno) | Najbolje za |
|------|----------------|-------------|
| **Ahrefs** | od 99 USD/mes | Backlink analiza, keyword research, site audit, konkurentska analiza. Najpopularniji SEO alat. |
| **SEMrush** | od 129 USD/mes | All-in-one: SEO + PPC + Social + Content. Odličan za kompetitivnu analizu |
| **Moz Pro** | od 49 USD/mes | Domain Authority, keyword tracking. Dobar za početnike |
| **Surfer SEO** | od 69 USD/mes | On-page optimizacija — analizira top rezultate i daje preporuke za sadržaj |
| **Screaming Frog** | Besplatan do 500 URL-ova, 259 GBP/god | Tehnički SEO audit — crawler koji pronalazi tehničke probleme |
| **Clearscope** | od 170 USD/mes | Content optimization — pomaže u pisanju SEO-optimizovanog sadržaja |
| **Mangools (KWFinder)** | od 29 USD/mes | Jednostavan keyword research alat. Budget-friendly alternativa Ahrefs-u |

### Besplatni

| Alat | URL | Namena |
|------|-----|--------|
| **Ubersuggest** | [neilpatel.com/ubersuggest](https://neilpatel.com/ubersuggest) | Keyword research (ograničeni besplatni plan) |
| **AnswerThePublic** | [answerthepublic.com](https://answerthepublic.com) | Pitanja koja ljudi postavljaju (ograničeno besplatno) |
| **Google Autocomplete** | [google.com](https://google.com) | Predlozi pretrage — besplatni keyword ideas |
| **Also Asked** | [alsoasked.com](https://alsoasked.com) | "People Also Ask" pitanja |
| **Check My Links** | Chrome extension | Pronalazi broken linkove na stranici |
| **Detailed SEO Extension** | Chrome extension | Vidi title, meta, heading-e na bilo kojoj stranici |
| **Yoast SEO** | WP plugin | WordPress SEO plugin — on-page optimizacija |

---

## 4. Content i kreativni alati

### Dizajn

| Alat | Cena | Namena |
|------|------|--------|
| **Canva** | Besplatan / Pro od 12 EUR/mes | Dizajn slika za oglase, social media, prezentacije. Najlakši za početnike |
| **Figma** | Besplatan / Pro od 12 USD/mes | Profesionalni dizajn alat za UI/UX i marketing materijale |
| **Adobe Creative Suite** | od 20 USD/mes per app | Photoshop, Illustrator — profesionalni dizajn |
| **Remove.bg** | Besplatan (ograničeno) | Uklanjanje pozadine sa slika |
| **TinyPNG** | Besplatan | Kompresija slika bez gubitka kvaliteta |
| **Squoosh** | [squoosh.app](https://squoosh.app) (besplatan) | Google-ov alat za kompresiju i konverziju slika (WebP, AVIF) |

### Video

| Alat | Cena | Namena |
|------|------|--------|
| **Loom** | Besplatan / Pro od 12.50 USD/mes | Snimanje screen recording-a sa kamerom — odličan za UGC demo videe |
| **CapCut** | Besplatan | Video editing — popularan za kratke formate (Reels, Stories) |
| **DaVinci Resolve** | Besplatan | Profesionalni video editing (kompleksniji, ali moćan) |
| **Descript** | od 24 USD/mes | Video editing baziran na tekstu — edituj video kao dokument |
| **Synthesia** | od 22 USD/mes | AI video generisanje sa avatarima |

### Copywriting

| Alat | Cena | Namena |
|------|------|--------|
| **ChatGPT / Claude** | Besplatan / Pro | Pomoć u pisanju oglasa, blog postova, email-ova |
| **Hemingway Editor** | Besplatan (web) | Poboljšanje čitljivosti teksta |
| **Grammarly** | Besplatan (basic) | Provera gramatike (engleski) |
| **CoSchedule Headline Analyzer** | Besplatan | Analiza kvaliteta naslova |

---

## 5. Landing page alati

| Alat | Cena | Namena |
|------|------|--------|
| **Unbounce** | od 99 USD/mes | Kreiranje i A/B testiranje landing stranica bez kodiranja |
| **Webflow** | od 14 USD/mes | Vizualni web builder — za marketing sajtove i LP |
| **Carrd** | od 19 USD/god | Jednostavne one-page sajtove i LP |
| **Framer** | Besplatan / Pro od 5 USD/mes | Moderni landing page builder |
| **Instapage** | od 79 USD/mes | Enterprise LP builder sa A/B testiranjem |
| **WordPress + Elementor** | Besplatan (hosting extra) | Najpopularnija kombinacija za sajtove i blog |

---

## 6. Email marketing

| Alat | Cena | Namena |
|------|------|--------|
| **Mailchimp** | Besplatan do 500 kontakata | Email marketing za početnike |
| **ConvertKit** | Besplatan do 1000 pretplatnika | Email za kreatore i SaaS — automatizacije |
| **Brevo (ex Sendinblue)** | Besplatan do 300 email-ova/dan | Email + SMS + CRM |
| **Customer.io** | od 100 USD/mes | Event-driven email za SaaS — onboarding sekvence |
| **ActiveCampaign** | od 29 USD/mes | Napredan email marketing sa CRM-om |
| **Loops** | Besplatan do 1000 kontakata | Email alat specifično dizajniran za SaaS |

---

## 7. Analitika i dashboarding

| Alat | Cena | Namena |
|------|------|--------|
| **Google Looker Studio** | Besplatan | Dashboarding — povezivanje sa Google alatima |
| **Databox** | Besplatan (3 datasource) | Lepši dashboards, automatski pull podataka |
| **Mixpanel** | Besplatan do 20M events | Product analytics — praćenje korisničkog ponašanja u app-u |
| **Amplitude** | Besplatan (ograničeno) | Product analytics — alternativa Mixpanel-u |
| **Hotjar** | Besplatan (ograničeno) | Heatmaps, session recordings, surveys — vidi kako se korisnici ponašaju na sajtu |
| **Microsoft Clarity** | Besplatan | Heatmaps i session recordings — Hotjar alternativa |
| **Posthog** | Besplatan (self-hosted) | Open source product analytics |

---

## 8. Konkurentska analiza

| Alat | Cena | Namena |
|------|------|--------|
| **Facebook Ad Library** | Besplatan | Vidi sve aktivne oglase bilo kog brenda na Meta platformi |
| **Google Ads Transparency Center** | Besplatan | Vidi Google oglase konkurencije |
| **SpyFu** | od 39 USD/mes | Konkurentski Google Ads i SEO podaci |
| **SimilarWeb** | Besplatan (ograničeno) | Analiza saobraćaja konkurencije |
| **BuiltWith** | Besplatan (ograničeno) | Vidi koje tehnologije koristi konkurent na sajtu |

---

## 9. UGC i kreativni sourcing

| Alat | Cena | Namena |
|------|------|--------|
| **Billo** | od 59 USD/video | Platforma za naručivanje UGC video oglasa |
| **Insense** | od 400 USD/mes | UGC marketplace + Meta Ads integracija |
| **Fiverr** | Zavisi od kreatora | Freelance platforma — traži "UGC creator" |
| **Upwork** | Zavisi od kreatora | Freelance platforma za kreatore i video editore |

---

## 10. CMP (Cookie Consent) alati

| Alat | Cena | Namena |
|------|------|--------|
| **Cookiebot** | Besplatan do 100 str. | Cookie consent baner + GDPR compliance |
| **CookieYes** | Besplatan (ograničeno) | Jednostavan cookie consent |
| **Iubenda** | Besplatan (ograničeno) | Cookie consent + Privacy Policy generator |
| **OneTrust** | Enterprise cena | Enterprise-grade consent management |

---

## 11. SaaS direktorijumi za listing (backlink-ovi + vidljivost)

| Platforma | URL | Napomena |
|-----------|-----|---------|
| **Product Hunt** | [producthunt.com](https://producthunt.com) | Lansiranje proizvoda, community feedback |
| **G2** | [g2.com](https://g2.com) | Business software reviews — bitan za B2B |
| **Capterra** | [capterra.com](https://capterra.com) | Software directory — jak backlink |
| **GetApp** | [getapp.com](https://getapp.com) | Software comparison |
| **AlternativeTo** | [alternativeto.net](https://alternativeto.net) | Rangira se za "[proizvod] alternativa" |
| **Crunchbase** | [crunchbase.com](https://crunchbase.com) | Startup directory |
| **SaaSHub** | [saashub.com](https://saashub.com) | SaaS-specifičan direktorijum |
| **AppSumo** | [appsumo.com](https://appsumo.com) | Lifetime deals — može doneti mnogo korisnika |
| **SourceForge** | [sourceforge.net](https://sourceforge.net) | Software directory — jak domen |
| **BetaList** | [betalist.com](https://betalist.com) | Za lansiranje beta verzije |
| **SaaSWorthy** | [saasworthy.com](https://saasworthy.com) | SaaS reviews i poređenja |

---

## 12. Resursi za učenje

### Blogovi i newsletteri

| Resurs | URL | Tema |
|--------|-----|------|
| **Ahrefs Blog** | [ahrefs.com/blog](https://ahrefs.com/blog) | SEO edukacija — najbolji SEO blog |
| **Backlinko** | [backlinko.com](https://backlinko.com) | SEO strategije — Brian Dean |
| **Search Engine Land** | [searchengineland.com](https://searchengineland.com) | SEO i PPC vesti |
| **Jon Loomer** | [jonloomer.com](https://jonloomer.com) | Facebook Ads napredni tutorijali |
| **Andrew Chen** | [andrewchen.com](https://andrewchen.com) | Growth strategije za startupe |
| **Demand Curve** | [demandcurve.com](https://demandcurve.com) | Growth marketing za startupe |
| **Lenny's Newsletter** | [lennysnewsletter.com](https://lennysnewsletter.com) | Product i growth za SaaS |
| **SaaStr** | [saastr.com](https://saastr.com) | Sve o SaaS biznisu |

### YouTube kanali

| Kanal | Tema |
|-------|------|
| **Ahrefs** | SEO tutorijali |
| **Surfside PPC** | Google Ads tutorijali |
| **Ben Heath** | Facebook Ads tutorijali |
| **Google Ads** (oficial) | Najnovije Google Ads funkcije |
| **Wes McDowell** | Digital marketing za mali biznis |

### Kursevi

| Kurs | Platforma | Cena |
|------|----------|------|
| Google Ads Certification | [skillshop.withgoogle.com](https://skillshop.withgoogle.com) | Besplatan |
| Meta Blueprint Certification | [facebook.com/business/learn](https://facebook.com/business/learn) | Besplatan |
| Google Analytics Certification | [skillshop.withgoogle.com](https://skillshop.withgoogle.com) | Besplatan |
| HubSpot Academy | [academy.hubspot.com](https://academy.hubspot.com) | Besplatan |
| SEMrush Academy | [semrush.com/academy](https://semrush.com/academy) | Besplatan |
| CXL Institute | [cxl.com](https://cxl.com) | Plaćen (napredan) |

---

## Preporučeni stack za SaaS startup koji počinje

### Minimum (besplatan/jeftin)

| Kategorija | Alat | Cena |
|-----------|------|------|
| Analytics | Google Analytics 4 | Besplatan |
| Tag Management | Google Tag Manager | Besplatan |
| SEO Monitoring | Google Search Console | Besplatan |
| Ads — Search | Google Ads | Pay per click |
| Ads — Social | Meta Ads Manager | Pay per click |
| Email | ConvertKit ili Loops | Besplatan do limita |
| Dizajn | Canva | Besplatan |
| Video | Loom + CapCut | Besplatan |
| Heatmaps | Microsoft Clarity | Besplatan |
| Dashboarding | Google Sheets ili Looker Studio | Besplatan |
| Cookie Consent | Cookiebot | Besplatan do 100 str. |

### Kada počneš da skaliraš (500+ EUR/mes na oglase)

Dodaj:
- **Ahrefs ili SEMrush** — za ozbiljan SEO
- **Hotjar** — za dublje razumevanje ponašanja na sajtu
- **Customer.io ili Loops** — za napredne onboarding email sekvence
- **Unbounce ili Webflow** — za profesionalne landing stranice sa A/B testiranjem
